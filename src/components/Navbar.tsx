      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            <Link href="/chat" className="text-white/70 hover:text-white transition-colors text-sm font-medium">
              Chat
            </Link>
            ...
            {session ? (
              <div className="relative">
                ...
                >
                  ...
                  <span className="text-sm text-white/80">{session.user?.name ?? session.user?.email?.split('@')[0]}</span>
      ...
      {open && (
        <div className="md:hidden glass-strong border-t border-white/5">
          <div className="px-4 py-4 flex flex-col gap-3">
            <Link href="/chat" onClick={() => setOpen(false)} className="text-white/80 hover:text-white py-2 text-sm font-medium">
              Chat
            </Link>
            ...
            {session ? (
              <>
                <Link href="/dashboard" onClick={() => setOpen(false)} className="text-white/80 hover:text-white py-2 text-sm font-medium">
                  Dashboard
                </Link>
                <button onClick={() => signOut({ callbackUrl: '/' })} className="text-red-400 py-2 text-sm font-medium text-left">
                  Déconnexion
                </button>
              </>
            ) : (
              <>
                <Link href="/login" onClick={() => setOpen(false)} className="text-white/80 hover:text-white py-2 text-sm font-medium">
                  Connexion
                </Link>
                <Link href="/register" onClick={() => setOpen(false)} className="btn-primary text-center text-sm">
                  S'inscrire gratuitement
                </Link>
              </>
            )}
